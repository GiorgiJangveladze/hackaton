...
$(function () {
  if(window.CAN_SEE_GLOSSARY) {
    var glossaries = $('.glossary-link');

    $.each(glossaries, function (k, glossary) {
      glossary = $(glossary);
      if(glossary.parents('.summernote-editor').length) return; // In the textarea

      var id = glossary.data().id;
      var definition = glossary.data().definition;

      glossary.attr({
        href: '/admin/glossaries/' + id,
        target: '_blank',
        'data-toggle': 'tooltip',
        title: definition
